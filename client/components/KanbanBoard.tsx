...
function TicketCard({ ticket, onSelect, overlay = false, disabled = false }: TicketCardProps) {
...
function KanbanColumn({ status, tickets, children }: { status: TicketStatus; tickets: Ticket[]; children: React.ReactNode }) {
  const droppable = useDroppable({ id: `column:${status}`, data: { type: 'column', status } });
  return (
    <section className={`kanban-column kanban-column--${status}`} aria-labelledby={`heading-${status}`}>
      <header>
        <span className="status-dot" aria-hidden="true" />
        <h2 id={`heading-${status}`}>{STATUS_LABELS[status]}</h2>
        <span className="column-count">{tickets.length}</span>
      </header>
      <div ref={droppable.setNodeRef} className={`kanban-column__body ${droppable.isOver ? 'is-over' : ''}`}>
...
export function KanbanBoard({ tickets, onSelect, onStatusChange, statusChangePending }: KanbanBoardProps) {
    ...
    >
      <section className="kanban-board" aria-label="Support ticket board">
        {TICKET_STATUSES.map((status) => (
          <KanbanColumn status={status} tickets={grouped[status]} key={status}>
